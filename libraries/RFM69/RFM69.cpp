...
bool RFM69::initialize(uint8_t freqBand, uint8_t nodeID, uint8_t networkID)
{
  const uint8_t CONFIG[] =
  {
...
#if 0	// Hard coding the frequency saves 50 bytes
    ...
    /* 0x30 */ // REG_SYNCVALUE2, networkID, // NETWORK ID, set below.
  ...
  do writeReg(REG_SYNCVALUE1, 0xAA); while (readReg(REG_SYNCVALUE1) != 0xaa && millis() < timeout);
	timeout = millis() + 50;
  do writeReg(REG_SYNCVALUE1, 0x55); while (readReg(REG_SYNCVALUE1) != 0x55 && millis() < timeout);

 // for (uint8_t i = 0; CONFIG[i][0] != 255; i++)
//    writeReg(CONFIG[i][0], CONFIG[i][1]);
	{
		const uint8_t*	configPtr = CONFIG;
		for (uint8_t command = *(configPtr++); command; command = *(configPtr++))
		{
			writeReg(command, *(configPtr++));
		}
	}
  // Encryption is persistent between resets and can trip you up during debugging.
  // Disable it during initialization so we always start from a known state.
#if USE_ENCRYPTION	// No need to #if out the actual encrypt function, the linker takes care of that.
  encrypt(0);
#endif
#if IS_RFM69HW
...
//	timeout = millis() + 50;
//  while (((readReg(REG_IRQFLAGS1) & RF_IRQFLAGS1_MODEREADY) == 0x00) && millis() < timeout); // wait for ModeReady
  if (millis() >= timeout)
    return false;
  setNetwork(networkID);
  _haveData = false;
#ifdef __AVR_ATtiny84__	// For the ATtiny84.  attachInterrupt will add 
	cli();
	MCUCR = (MCUCR & ~(_BV(ISC00) | _BV(ISC01))) | (RISING << ISC00);		// Sets the mode to RISING
	GIMSK |= _BV(INT0); // Turns on INT0
	sei();
#else
  attachInterrupt(0, RFM69::isr0, RISING);	// Costs 100 bytes due to duplicate vector table when used.
#endif
 // selfPointer = this;
 // setNetwork(networkID);
  _address = nodeID;
...
void RFM69::setMode(uint8_t newMode)
{
	...
	{
	#if 1
		uint8_t	reg = readReg(REG_OPMODE) & 0xE3;
		reg |= newMode;	// To reduce code size the RF69_MODEs = RF_OPMODEs
		...
		switch (newMode)
		{
			case RF69_MODE_TX:
			  ...
			  if (_isRFM69HW) setHighPowerRegs(true);
				...
				break;
			case RF69_MODE_RX:
				reg |= RF_OPMODE_RECEIVER;
			#if IS_RFM69HW
			  if (_isRFM69HW) setHighPowerRegs(false);
			#endif
				break;
			case RF69_MODE_SYNTH:
				reg |= RF_OPMODE_SYNTHESIZER;
				break;
			case RF69_MODE_STANDBY:
				reg |= RF_OPMODE_STANDBY;
			  break;
...
void RFM69::interruptHandler() {
  ...
  {
	//readReg(REG_TESTDAGC);	// For debugging to show that we got here
    //RSSI = readRSSI();
    setMode(RF69_MODE_STANDBY);
    select();
